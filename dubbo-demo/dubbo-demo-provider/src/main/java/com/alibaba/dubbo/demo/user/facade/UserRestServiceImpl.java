...
//        System.out.println("Client address from RpcContext: " + RpcContext.getContext().getRemoteAddressString());
    ...
    @POST
    @Path("register")
    public RegistrationResult registerUser(@BeanParam User user) {
        return new RegistrationResult(userService.registerUser(user));
    }

    @GET
    @Path("{application:\\w+}/{id : \\d+}")
    public User getUser(@PathParam("id") Long id, @PathParam("application") String application) {
        return userService.getUser(id);
    }
}
*/
