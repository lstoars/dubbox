...
public class Services extends Restful {
	...
	public void index(Map<String, Object> context) {
		...
		Map<String, Integer> providerCount = new HashMap<String, Integer>();
		...
		for (String service : providerServices) {
			for (Provider p : providers) {
				if (service.equals(p.getService())) {
					providerCount.put(service, providerCount.get(service) == null ? 1 : providerCount.get(service) + 1);
				}
			}
		}

		for (String service : consumerServices) {
			for (Consumer c : consumers) {
				if (service.equals(c.getService())) {
					consumerCount.put(service, consumerCount.get(service) == null ? 1 : consumerCount.get(service) + 1);
				}
			}
		}
		...
		context.put("consumerCount",consumerCount);
